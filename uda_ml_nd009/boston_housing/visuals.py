"""
Create by Killer at 2019-05-31 14:33
"""
...
def ModelLearning(X, y):
    ...
    for k, depth in enumerate([1, 3, 6, 10]):
        ...
        ax.set_xlabel('Number of Traning Points')
    ...
    fig.suptitle('Decision Tree Regressor Performances', fontsize=16, y=1.03)
    fig.tight_layout()
    fig.show()
...
def ModelComplexity(X, y):
    ...
    # Create 10 cross-validation sets for traning and testing
    cv = ShuffleSplit(n_splits=10, test_size=0.2, random_state=0)
    ...
    valid_std = np.mean(valid_scores, axis=1)

    # Plot the validation curve
    plt.figure(figsize=(7, 5))
    plt.title('Decision Tree Regressor Complexity Performance')
    plt.plot(max_depth, train_mean, 'o-', color='r', label='Training Score')
    plt.plot(max_depth, valid_mean, 'o-', color='g', label='Validating Score')
